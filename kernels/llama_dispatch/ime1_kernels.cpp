...
void quantize_a_4row_i8(size_t BlkLen, const float *A, size_t CountK, std::byte *QuantA) {
    ...
    if (BlkLen == 16 || BlkLen == 32 || BlkLen == 64) {
        for (size_t row_index = 0; row_index < 4; ++row_index) {
            ...
            __asm__ volatile("vsetvli            t0, zero, e32, m8, ta, ma\n\t"
                             "addi               t2, %[CountK], 0         \n\t"
                             "addi               a1, %[DST], 0            \n\t"
                             "blt                t2, %[BlkLen], TAIL%=    \n\t"

                             "LOOP%=:                                     \n\t"
                             "vsetvli            t0, %[BlkLen], e32, m8, ta, ma\n\t"
                             "vle32.v            v0, (%[SRC])             \n\t"
                             "sub                t2, t2, t0               \n\t"
                             "slli               t1, t0, 2                \n\t"
                             "add                %[SRC], %[SRC], t1       \n\t"
                             ...
                             "QUIT%=:                                     \n\t"
                             : [SRC] "+r"(SRC)
                             : [DST] "r"(DST), [BlkLen] "r"(BlkLen), [OFFSET] "r"(offset), [STRIDE] "r"(stride), [CountK] "r"(CountK), [FONE] "f"(fone), [RMAXREC] "f"(range_max_reciprocal)
...
void quantize_a_row_i8(size_t BlkLen, const float *A, size_t CountK, std::byte *QuantA) {
    ...
    if (CountK <= BlkLen) {
        ...
        for (size_t k = 0; k < CountK; k++) {
            QuantAData_offset[k] = (int8_t)std::clamp(roundf(A[k] / scale_A), (float)std::numeric_limits<int8_t>::lowest(), (float)std::numeric_limits<int8_t>::max());
        }
    ...
    if (BlkLen == 16) {
        ...
        __asm__ volatile("addi         t3, zero, 16*8          \n\t"
                         "addi         t2, zero, 16            \n\t"
                         "blt          %[K], t3, LOOP_K%=      \n\t"
                         "blt          %[K], t2, TAIL%=        \n\t"
                         "LOOP_MAIN%=:                         \n\t"
                         ...
                         "TAIL%=:                              \n\t"
                         "blez         %[K], END%=             \n\t"
                         "vsetvli      t0, t3, e32, m2, ta, ma \n\t"
                         "vxor.vv      v16, v16, v16           \n\t"
                         "jal          x0, LOOP_K%=            \n\t"
                         "END%=:                               \n\t"
                         : [SRC] "+r"(SRC), [DST] "+r"(DST), [K] "+r"(CountK)
                         : [FONE] "f"(fone), [RMAXREC] "f"(range_max_reciprocal), [BUFFER] "r"(buffer)
                         : "cc", "t3", "t2", "t1", "t0", "a1", "f0", "f1", "f2", "f3", "f4", "f5", "f6", "f7", "f10", "f11", "f12", "f13", "f14", "f15", "f16", "f17");
    } else if (BlkLen == 32) {
        __asm__ volatile("addi         t3, zero, 32*4          \n\t"
                         "addi         t2, zero, 32            \n\t"
        ...
        __asm__ volatile("addi         t3, zero, 64*2          \n\t"
                         "addi         t2, zero, 64            \n\t"
                         "addi         a1, %[SRC], 0           \n\t"
                         "addi         a2, %[SRC], 256         \n\t"
                         "addi         s1, %[DST], 0           \n\t"
        ...
        __asm__ volatile("addi         t2, zero, 128           \n\t"
                         "addi         a1, %[SRC], 0           \n\t"
                         "addi         a2, %[SRC], 256         \n\t"
                         "blt          %[K], t2, TAIL%=        \n\t"
                         "LOOP_K%=:                            \n\t"
        ...
        __asm__ volatile("slli         t3, %[BLK], 2           \n\t"
                         "blt       %[K], %[BLK], LOOP_TAIL%=  \n\t"
                         "LOOP_MAIN%=:                         \n\t"
                         "vsetvli      t0, zero, e32, m1, ta, ma       \n\t"
                         "vxor.vv      v31, v31, v31           \n\t"
                         ...
                         "END%=:                               \n\t"
                         : [SRC] "+r"(SRC), [DST] "+r"(DST), [K] "+r"(CountK)
                         : [FONE] "f"(fone), [RMAXREC] "f"(range_max_reciprocal), [BLK] "r"(BlkLen), [BUFFER] "r"(buffer), [CNT] "r"(cnt)
...
void SQ4BitGemmM4Kernel_CompInt8_ScaleFp16_Impl(size_t BlkLen, const std::byte *QuantA, const std::byte *QuantBData, const float *QuantBScale, const std::byte *QuantBZeroPoint, float *C,
                                                size_t CountN, size_t BlockCountK, const float *Bias, const size_t ldc) {
    ...
    if constexpr (HasZeroPoint) {
        for (size_t n = 0; n < CountN; n += 16) {
            ...
            if (Bias != nullptr) {
                ...
                if (NBLKS < 16) {
                    __asm__ volatile("vsetvli        t0, %[N], e32, m2, ta, ma     \n\t"
                                     "vle32.v        v0, (%[SRC])          \n\t"
                                     "vse32.v        v0, (%[DST])          \n\t"
                                     :
                ...
                __asm__ volatile(LOAD_BIAS
                                 ...
                                 :
                                 : [INNER] "r"(INNER), [A] "r"(QuantA), [B] "r"(QuantBDataPtr), [LDC] "r"(LDC), [BlockCountK] "r"(BlockCountK), [C] "r"(CPtr), [BIAS] "r"(bias)
...
template <bool HasZeroPoint>
void SQ4BitGemmM4Kernel_CompInt8_Impl(size_t BlkLen, const std::byte *QuantA, const std::byte *QuantBData, const float *QuantBScale, const std::byte *QuantBZeroPoint, float *C, size_t CountN,
                                      size_t BlockCountK, const float *Bias, const size_t ldc) {
    GGML_UNUSED(QuantBScale);
    GGML_UNUSED(QuantBZeroPoint);
    size_t LDC = ldc * sizeof(float);
    const size_t INNER = BlkLen / 16;
    ...
    if constexpr (HasZeroPoint) {
        for (size_t n = 0; n < CountN; n += 16) {
            ...
            if (Bias != nullptr) {
                ...
                __asm__ volatile(LOAD_BIAS "addi               t3, %[BlockCountK], 0       \n\t"
                                           "vsetvli            t0, zero, e8, m1, ta, ma            \n\t"
                                           "li                 s1, 24                      \n\t"
                                           "vmv.v.i            v1, 3                       \n\t"
                                           "vsetvli            t0, s1, e8, m1, ta, ma              \n\t"
...
template <bool HasZeroPoint>
void SQ4BitGemmM1Kernel_CompInt8_ScaleFp16_Impl(size_t BlkLen, const std::byte *QuantA, const std::byte *QuantBData, const float *QuantBScale, const std::byte *QuantBZeroPoint, float *C,
                                                size_t CountN, size_t BlockCountK, const float *Bias) {
    GGML_UNUSED(QuantBScale);
    GGML_UNUSED(QuantBZeroPoint);
    ...
    if constexpr (HasZeroPoint) {
        for (size_t n = 0; n < CountN; n += 16) {
            ...
            if (Bias != nullptr) {
                const float *bias = Bias + n;
                __asm__ volatile("addi         t3, %[NBLKS], 0         \n\t"
                                 "vsetvli      t0, zero, e8, m1, ta, ma        \n\t"
                                 ...
                                 "ST_TAIL%=:                           \n\t"
                                 "vsetvli      t0, %[NBLKS], e32, mf2, ta, ma  \n\t"
                                 "sub          %[NBLKS], %[NBLKS], t0  \n\t"
                                 "vse32.v      v28, (%[C])             \n\t"
                                 "vsetvli      t0, %[NBLKS], e32, mf2, ta, ma  \n\t"
                                 "sub          %[NBLKS], %[NBLKS], t0  \n\t"
...
template <bool HasZeroPoint>
void SQ4BitGemmM1Kernel_CompInt8_Impl(size_t BlkLen, const std::byte *QuantA, const std::byte *QuantBData, const float *QuantBScale, const std::byte *QuantBZeroPoint, float *C, size_t CountN,
                                      size_t BlockCountK, const float *Bias) {
    GGML_UNUSED(QuantBScale);
    GGML_UNUSED(QuantBZeroPoint);
    const size_t INNER = BlkLen / 16;
    if constexpr (HasZeroPoint) {
...
template <bool HasZeroPoint>
inline void SQ4BitGemmM4Kernel_CompInt8_DispatchOnBlkLen(size_t BlkLen, const std::byte *QuantA, const std::byte *QuantBData, const float *QuantBScale, const std::byte *QuantBZeroPoint, float *C,
                                                         size_t CountN, size_t BlockStrideQuantB, const float *Bias, const size_t ldc, const size_t scalestride) {
    if (scalestride == 4) {
        SQ4BitGemmM4Kernel_CompInt8_Impl<HasZeroPoint>(BlkLen, QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountN, BlockStrideQuantB, Bias, ldc);

    } else if (scalestride == 2) {
        SQ4BitGemmM4Kernel_CompInt8_ScaleFp16_Impl<HasZeroPoint>(BlkLen, QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountN, BlockStrideQuantB, Bias, ldc);
    }
}

template <bool HasZeroPoint>
inline void SQ4BitGemmM1Kernel_CompInt8_DispatchOnBlkLen(size_t BlkLen, const std::byte *QuantA, const std::byte *QuantBData, const float *QuantBScale, const std::byte *QuantBZeroPoint, float *C,
                                                         size_t CountN, size_t BlockStrideQuantB, const float *Bias, const size_t scalestride) {
    if (scalestride == 4) {
        SQ4BitGemmM1Kernel_CompInt8_Impl<HasZeroPoint>(BlkLen, QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountN, BlockStrideQuantB, Bias);
    } else if (scalestride == 2) {
        SQ4BitGemmM1Kernel_CompInt8_ScaleFp16_Impl<HasZeroPoint>(BlkLen, QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountN, BlockStrideQuantB, Bias);
    }
}
...
namespace ime1 {
size_t gemm_kernel_i8i4(size_t BlkLen, const std::byte *QuantA, const std::byte *QuantBData, const float *QuantBScale, const std::byte *QuantBZeroPoint, float *C, size_t CountM, size_t CountN,
                        size_t CountK, size_t BlockCountK, size_t ldc, const float *Bias, const size_t ScaleStride) {
    GGML_UNUSED(CountK);
    if (CountM >= 4) {
        if (QuantBZeroPoint != nullptr) {
            SQ4BitGemmM4Kernel_CompInt8_DispatchOnBlkLen<true>(BlkLen, QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountN, BlockCountK, Bias, ldc, ScaleStride);
        } else {
            SQ4BitGemmM4Kernel_CompInt8_DispatchOnBlkLen<false>(BlkLen, QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountN, BlockCountK, Bias, ldc, ScaleStride);
        }
        return 4;
    } else {
        if (QuantBZeroPoint != nullptr) {
            SQ4BitGemmM1Kernel_CompInt8_DispatchOnBlkLen<true>(BlkLen, QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountN, BlockCountK, Bias, ScaleStride);
        } else {
            SQ4BitGemmM1Kernel_CompInt8_DispatchOnBlkLen<false>(BlkLen, QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountN, BlockCountK, Bias, ScaleStride);
        }
        return 1;
    }
}
