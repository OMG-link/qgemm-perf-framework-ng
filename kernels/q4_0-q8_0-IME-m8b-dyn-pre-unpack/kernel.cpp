...
void SQ4BitGemmM8Kernel_CompInt8_ScaleFp16_Impl_Intrin_BatchRed_DynPreUnpack(
    const uint8_t *GGML_RESTRICT baseA, const int8_t *GGML_RESTRICT baseWQs,
    const uint16_t *GGML_RESTRICT baseWScales, float *GGML_RESTRICT baseC,
    size_t CountN, size_t BlockCountK, const size_t ldc) {
...
#define STORE_REDUCTION_ACCUMULATOR(index, unused) __riscv_vse32_v_f32m2(acc + index * kOutputN, reduction_acc##index, ime::rvv::vl(32, 2));
            ...
            for (size_t reductionIndex = 0; reductionIndex < reductionCount; ++reductionIndex) {
                ...
                vfloat16m1_t bsh = __riscv_vle16_v_f16m1(
                    (_Float16 *)(baseWScales + (n / kOutputN) * BlockCountK * kOutputN +
                                 reductionBlock * kOutputN),
                    ime::rvv::vl(16, 1));
